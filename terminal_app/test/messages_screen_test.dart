...
import 'package:local_pos/src/services/system_notification_service.dart';
...
class _FakeDatabase extends LocalDatabaseService {
  final List<PosNotification> rows = [];
  ...
  Future<void> upsertNotification(PosNotification notification) async {
    rows
      ..removeWhere((row) => row.id == notification.id)
...
class _FakeSystemNotifications extends SystemNotificationService {
  @override
  Future<void> show({
    required int id,
...
void main() {
  ...
  testWidgets(
    'messages inbox queues websocket refresh while fetch is in flight',
    (tester) async {
      ...
      expect(api.fetchCount, 2);
      second.complete([_chat(message: 'queued inbox refresh')]);
      await tester.pump();
      await tester.pump();

      expect(find.text('queued inbox refresh'), findsOneWidget);
  ...
  test(
    'chat websocket creates notification only for needs-help chats',
    () async {
      final database = _FakeDatabase();
      final api = _FakeChatApi([
        [
          _chat(
            status: ChatStatus.needs,
            message: 'Can I see a Kacchi photo?',
            reason: 'asked for a Kacchi photo',
          ),
        ],
        [_chat(status: ChatStatus.replied, message: 'Manager answered')],
        [_chat(status: ChatStatus.bot, message: 'Bot is handling this')],
      ]);
      final controller = _controller(
        api,
        database: database,
        systemNotifications: _FakeSystemNotifications(),
      );

      controller.debugHandleRemoteSyncEvent({
        'type': 'chat_updated',
        'data': {'conversationId': 'chat-1'},
      });
      ...
      expect(database.rows.single.title, 'Chatbot needs you');
      expect(database.rows.single.body, 'Rumana: asked for a Kacchi photo');
      expect(database.rows.single.actionTarget, 'messages');
  ...
  test('chat escalation notification dedupe resets after handback', () async {
    final database = _FakeDatabase();
    final api = _FakeChatApi([
      [_chat(status: ChatStatus.needs, message: 'Need help')],
      [_chat(status: ChatStatus.bot, message: 'Back with bot')],
      [_chat(status: ChatStatus.needs, message: 'Need help again')],
    ]);
    final controller = _controller(
      api,
      database: database,
    ...
    expect(database.rows.first.title, 'Chatbot needs you');
    expect(database.rows.first.body, 'Rumana: Need help again');
