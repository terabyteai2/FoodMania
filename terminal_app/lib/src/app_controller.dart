...
class PosAppController extends ChangeNotifier {
  ...
  @visibleForTesting
  void debugHandleRemoteSyncEvent(Map<String, Object?> event) {
    _handleRemoteSyncEvent(event);
  }
  ...
  /// Settings on the terminal holds printer + cloud config the manager needs,
  /// so it is reachable by manager (and owner). Owner-only sections were removed.
  bool get canManageSettings => isManager;

  /// Messenger chat takeover — owner & manager.
  bool get canMessages => isManager;

  /// Menu management — owner & manager (waiters view menu only).
  ...
  String? get _currentServerName {
    ...
    final username = accountUsername.trim();
  ...
  Future<void> addNotification({
    ...
    final bulkCount =
        notifications
            ...
            .length +
        1;
    final keepSystemTargetTitle =
        type == PosNotificationType.system && actionTarget == 'messages';
    final displayTitle = bulkCount > 1 && !keepSystemTargetTitle
