...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      ...
      actions: [
        ...
        PrimaryButton(
          label: text.isBn ? 'দিন শেষ' : 'End day',
          icon: Icons.receipt_long_outlined,
          secondary: true,
          busy: _printingThermal,
          onPressed: report.totalOrders == 0 ? null : _printThermalEndOfDay,
        ),
      ],
      child: Column(
  ...
  Future<void> _printThermalEndOfDay() async {
    final app = AppScope.of(context);
    setState(() => _printingThermal = true);
    try {
      final posReport = await app.desktopPosReport(days: 1);
      final shift = await app.currentDesktopShift();
      final ok = await app.printEndOfDayReport(report: posReport, shift: shift);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: TfText(
            ok
                ? (app.strings.isBn
                      ? 'দিন শেষ রিপোর্ট প্রিন্ট হয়েছে'
                      : 'End of day report printed')
                : app.strings.printFailed,
          ),
        ),
      );
    } catch (error) {
...
class _PeriodSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return TfCard(
      padding: EdgeInsets.all(PosSpacing.s6),
      child: SegmentedButton<int>(
        style: ButtonStyle(
          visualDensity: VisualDensity.compact,
          textStyle: WidgetStatePropertyAll(
            TextStyle(fontSize: PosFont.f11, fontWeight: FontWeight.w500),
          ),
        ),
        segments: [
          ButtonSegment(
...
class _ReportTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TfCard(
      clip: true,
      padding: EdgeInsets.all(PosSpacing.s8),
      child: Row(
        children: [
          Container(
            width: 32,
            ...
            child: Icon(icon, color: PosColors.primaryDark, size: 16),
          ),
          SizedBox(width: PosSpacing.s8),
          Expanded(
            child: Column(
...
class _ReportCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TfCard(
      padding: EdgeInsets.all(PosSpacing.s10),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
...
class _LineRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(vertical: PosSpacing.s5),
      child: Row(
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                TfText(
                  ...
                  style: TextStyle(
                    color: PosColors.primaryDark,
                    fontSize: PosFont.f12,
                    fontWeight: FontWeight.w500,
                    height: 1,
                  ),
                ),
                SizedBox(height: PosSpacing.s2),
                TfText(
                  subtitle,
                  style: TextStyle(
