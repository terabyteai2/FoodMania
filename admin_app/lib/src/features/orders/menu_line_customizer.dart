...
class MenuLineOption {
  const MenuLineOption({required this.label, this.priceDelta = 0});
...
const MenuLineOption _baseMenuOption = MenuLineOption(label: '');

class MenuLineSelection {
  const MenuLineSelection({
    required this.item,
...
Future<MenuLineSelection?> showMenuLineCustomizer(
  BuildContext context, {
  required MenuItem item,
  required bool isBn,
  int initialQty = 1,
}) async {
  final selections = await showMenuLineCustomizerLines(
    context,
    item: item,
    isBn: isBn,
...
Future<List<MenuLineSelection>?> showMenuLineCustomizerLines(
  BuildContext context, {
  required MenuItem item,
  required bool isBn,
  int initialQty = 1,
  ...
  debugPrint(
    '[POS] Showing menu customizer for item=${item.name} '
    'id=${item.id} tags=${item.tags}',
  );
  return showDialog<List<MenuLineSelection>>(
    context: context,
...
String menuLineMoney(num value, {int decimals = 0}) {
...
TextStyle _numStyle(
  ...
  Color color = PosColors.primaryDark,
}) {
  return TextStyle(
    fontFamily: 'Inter',
    fontSize: size,
...
TextStyle _mutedMono(double size) {
  return TextStyle(
    fontFamily: 'Inter',
    fontSize: size,
...
List<MenuLineOption> menuLineOptionsFor(MenuItem item) {
  final parsed = configuredMenuLineOptionsFor(item);
  if (parsed.isNotEmpty) return parsed;
  return const [_baseMenuOption];
}

List<MenuLineOption> configuredMenuLineOptionsFor(MenuItem item) {
  return _dedupeOptions([
    for (final option in item.extras.options)
      MenuLineOption(label: option.name, priceDelta: option.priceDelta),
  ]);
}
...
List<MenuLineOption> _dedupeOptions(List<MenuLineOption> options) {
  final seen = <String>{};
  final out = <MenuLineOption>[];
...
class MenuLineThumb extends StatelessWidget {
  const MenuLineThumb({
    required this.item,
    this.size = 44,
    this.radius = 8,
...
class _MenuLineCustomizerDialogState extends State<_MenuLineCustomizerDialog> {
  ...
  late final MenuLineOption _option = _options.length > 1
      ? _options.firstWhere(
          ...
          child: Column(
            ...
            children: [
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  MenuLineThumb(item: widget.item, size: 62, radius: 10),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${menuLineMoney(widget.item.price)} base',
                          style: _mutedMono(12),
                        ),
                      ],
                    ),
                ...
                for (var i = 0; i < addOns.length; i++)
                  CheckboxListTile(
                    ...
                    secondary: addOns[i].price == 0
                        ? null
                        : Text('+${menuLineMoney(addOns[i].price)}'),
              ...
              Row(
                children: [
                  ...
                  const Spacer(),
                  Text(menuLineMoney(lineTotal), style: _numStyle(18)),
                ],
              ),
            ],
          ),
  ...
  List<MenuLineSelection> _selectedLines(List<MenuAddOn> addOns) {
