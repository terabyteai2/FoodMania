...
class _DashT1CounterState extends State<_DashT1Counter>
  ...
  List<MenuItem> _filteredMenuItems(List<MenuItem> all) {
    if (_searchQuery.isEmpty) return all;
    final q = _searchQuery.toLowerCase();
    return all.where((item) {
      return item.name.toLowerCase().contains(q) ||
          (item.categoryEn?.toLowerCase().contains(q) ?? false) ||
  ...
  Widget build(BuildContext context) {
    ...
    final earnedToday = money?.earnedToday ?? 0;
    final ordersToday = money?.kpis.orders ?? 0;
    final openCount = money?.kpis.openOrders ?? 0;
    final inFlight = right?.todaySoFarBdt ?? 0;
    final storeName = _outletTitle(app);
    final ringupItems = _topSellerItems(app, summary);
    final filteredItems = _filteredMenuItems(ringupItems);
    ...
    return _manageScaffold(
      ...
      children: [
        _section(
          top: 16,
          child: _FcSummaryCard(
            storeName: storeName,
            earnedToday: _bdt(earnedToday),
            ordersToday: ordersToday,
            openCount: openCount,
            inFlight: _bdt(inFlight),
          ),
        ),
        _section(
          top: 16,
          child: _FcSearchField(
        ...
        _section(
          ...
          child: _FcRingupGrid(
            ...
            onTap: (item) => unawaited(_increment(item)),
            onEdit: () =>
                unawaited(_openQuickSellEditor(context, app, summary)),
          ),
        ),
      ],
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// FOODCART MODE — new v2 widgets for _DashT1Counter
// ─────────────────────────────────────────────────────────────────────────────

class _FcSummaryCard extends StatelessWidget {
  const _FcSummaryCard({
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Container(width: 1, height: 48, color: PosColors.divider),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const _FcEyebrow(text: 'Open now'),
                    const SizedBox(height: 4),
                    Text(
                      '$openCount',
                      style: const TextStyle(
...
class _FcEyebrow extends StatelessWidget {
  const _FcEyebrow({required this.text});
  final String text;
...
class _FcSearchField extends StatefulWidget {
  ...
  @override
  State<_FcSearchField> createState() => _FcSearchFieldState();
}

class _FcSearchFieldState extends State<_FcSearchField> {
  bool _hasText = false;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.search, size: 20, color: PosColors.muted),
          const SizedBox(width: 12),
          Expanded(
            child: TextField(
          ...
          if (_hasText)
            GestureDetector(
              ...
              child: Icon(Icons.close, size: 20, color: PosColors.mutedSoft),
            ),
        ],
      ),
    );
...
class _FcRecentRow extends StatelessWidget {
  const _FcRecentRow({required this.items, required this.onTap});
...
class _FcRingupGrid extends StatelessWidget {
  const _FcRingupGrid({
    required this.items,
...
class _FcRingupTile extends StatelessWidget {
  const _FcRingupTile({
    required this.item,
    required this.qty,
    required this.onTap,
