    ...
    return Scaffold(
      ...
      appBar: _mode == _Mode.preview
          ? AppBar(
              ...
              title: TfText(text.scanStock, style: TfTextStyles.appBarTitle),
            )
          : null,
      ...
      bottomNavigationBar: _mode == _Mode.preview
          ? TfStickyCTA(
              child: TfButton(
                label: text.stockScanPages(_pages.length),
...
class _SThumbnailTile extends StatelessWidget {
  const _SThumbnailTile({required this.child, this.onTap});
  final Widget child;
...
class _SPageThumbnail extends StatelessWidget {
  const _SPageThumbnail({
    required this.thumb,
