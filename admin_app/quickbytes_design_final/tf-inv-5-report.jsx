...
const ProductRow = ({ rank, name, bn, qty, rev, top }) => (
  <div style={{
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ fontSize: 11, color: DASH.textTer, marginTop: 2, fontFamily: DASH.bn }}>{bn}</div>
    </div>
  ...
  <InvScreen padBottom={70 + 8}>
    ...
    <Section top={14}>
      ...
      <DashCard padded={false} style={{ overflow: 'hidden' }}>
        <ProductRow top rank={1} name="Beef Tehari" bn="বিফ তেহারি"   qty={42} rev="৳2,520" />
        <ProductRow rank={2} name="Chicken Burger" bn="চিকেন বার্গার" qty={28} rev="৳1,680" />
        <ProductRow rank={3} name="Plain Rice"    bn="সাদা ভাত"     qty={31} rev="৳930"   />
        <ProductRow rank={4} name="Borhani"       bn="বোরহানি"        qty={36} rev="৳720"   />
        <ProductRow rank={5} name="Singara"       bn="সিঙ্গাড়া"        qty={28} rev="৳569"   />
      </DashCard>
    </Section>
    ...
    <Section top={14}>
      ...
      <div style={{
        ...
        {[
          { l: 'M', n: 'Mutton',  bn: 'খাসি', onHand: '0.8', unit: 'kg', threshold: '1.0', supplier: 'Karim Goshto' },
          { l: 'G', n: 'Ginger',  bn: 'আদা',   onHand: '1.4', unit: 'kg', threshold: '2.0', supplier: 'Karwan Bazar' },
        ].map((it, i) => (
          <div key={i} style={{
            padding: '12px 14px',
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: DASH.text, lineHeight: 1.2 }}>
                {it.n}
                <span style={{ fontFamily: DASH.bn, color: DASH.textTer, fontWeight: 400, marginLeft: 6, fontSize: 11 }}>{it.bn}</span>
              </div>
