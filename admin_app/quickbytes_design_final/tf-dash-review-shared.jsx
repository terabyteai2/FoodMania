...
const ReviewTabs = ({ active = 'review', period }) => (
  <div style={{
    padding: '6px 16px 0', display: 'flex', alignItems: 'center', gap: 12,
  }}>
    <div style={{
      display: 'flex', background: RDASH.surfaceAlt, borderRadius: 10, padding: 3,
      ...
      {[
        ...
        return (
          <div key={t.id} style={{
            ...
            background: on ? RDASH.surface : 'transparent',
            color: on ? RDASH.text : RDASH.textSec,
            border: on ? `1px solid ${RDASH.borderStrong}` : '1px solid transparent',
    ...
    {period && (
      <div style={{
        ...
        padding: '6px 10px', height: 32, borderRadius: 10,
        background: RDASH.surface, border: `1px solid ${RDASH.border}`,
        fontSize: 11.5, fontWeight: 600, color: RDASH.text,
      }}>
        <span style={{ width: 6, height: 6, borderRadius: 3, background: RDASH.accent }} />
        {period}
        <InvIcon kind="chevronDown" s={13} color={RDASH.textSec} />
      </div>
    )}
  </div>
);
  ...
  <div style={{
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11, color: RDASH.textTer, marginTop: 4, fontFamily: RDASH.bn }}>আজকের আয়</div>
      </div>
