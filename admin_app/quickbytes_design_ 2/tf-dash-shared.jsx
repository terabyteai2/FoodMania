...
// Bottom nav — gets the accent color per the latest brief.
// Active item: accentSoft pill behind icon + accent label below; selected-state
// surface keeps the dominance rule intact (the FAB / sticky CTA stays the
// single full-saturation accent in any viewport).
      ...
      {list.map((it, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            color: on ? DASH.accent : DASH.textTer
          }}>
            <div style={{
              width: 44, height: 26, borderRadius: 999, display: 'grid', placeItems: 'center',
              background: on ? DASH.accentSoft : 'transparent',
              color: on ? DASH.accent : DASH.textTer
            }}>
              <NavIcon kind={it.kind} />
            </div>
            <div style={{ fontSize: 10.5, fontWeight: on ? 700 : 500, lineHeight: 1, color: on ? DASH.accent : DASH.textTer }}>
              {it.label}
            </div>
          </div>);
...
const FOHTile = ({ emoji, n, p, tint, qty, onClick }) => {
  const active = !!qty;
  return (
    <div onClick={onClick} style={{
      ...
      borderRadius: 12, padding: '10px 10px', minHeight: 110,
      display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
      color: DASH.ink, cursor: 'pointer'
    }}>
      {active &&
      <div style={{
      ...
      <div>
        <div style={{ fontSize: 12.5, fontWeight: 600, lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{n}</div>
        <div style={{ fontSize: 12, fontWeight: 600, color: DASH.textSec, marginTop: 3, ...DASH.num }}>{p}</div>
      </div>
    </div>);
...
const FOHCounter = ({ title = 'Counter · top sellers', tiles, cols = 3 }) =>
<div>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, padding: '0 2px' }}>
      ...
      fontSize: 11, fontWeight: 700, fontFamily: DASH.mono, letterSpacing: 0.5,
      display: 'inline-flex', alignItems: 'center', gap: 5
    }}>
        <span style={{ width: 5, height: 5, borderRadius: 3, background: DASH.accent }} />EDIT
      </div>
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, minmax(0,1fr))`, gap: 8 }}>
      {tiles.map((t, i) => <FOHTile key={i} {...t} />)}
    </div>
  </div>;
...
const TABLE_STATES = {
  ...
  seated: { bg: DASH.accentSoft, border: DASH.accentSoft, dot: DASH.accentSoftInk, fg: DASH.accentSoftInk, label: 'Seated', bn: '' },
  kitchen: { bg: DASH.goodSoft, border: DASH.good, dot: DASH.good, fg: DASH.good, label: 'In kitchen', bn: '' },
  ...
  bill: { bg: DASH.surfaceAlt, border: DASH.borderStrong, dot: DASH.inkRaised, fg: DASH.inkRaised, label: 'Bill pending', bn: '' },
