...
// Reads ?lang=bn from the current URL. Falls back to 'en'.
function readLang() {
...
// Build a URL with the opposite lang flag, preserving other query params.
...
// Pick `field` from item using the requested language with graceful fallback.
//   pick(item, 'name', 'bn') → item.nameBn || item.nameEn || item.name
...
// Fixed UI strings used by Hearth components.
...
// Font family for a text run in the active lang. Bangla → Hind Siliguri.
export function fontFor(lang, baseFamily) {
  if (lang === 'bn') return '"Hind Siliguri", system-ui, sans-serif'
  return baseFamily
}
