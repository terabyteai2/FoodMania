...
def _batched_system_prompt() -> str:
    return (
        ...
        "MIRROR THE CUSTOMER'S TONE: match their register, energy, and style — if they're casual and "
        "terse, reply casual and short; if formal, be polite and complete; if they use emojis, "
        "exclamations, or slang, match that level; let your message length track theirs. Sound like a "
        "real person texting on Messenger, not a corporate helper or salesperson. Don't be "
        "over-effusive when the customer is matter-of-fact, and don't be stiff when they're playful. "
        "Use common sense. \n"
...
def _final_reply_system_prompt() -> str:
    return (
        ...
        "You MUST reply in conversational Bangla using the Bangla alphabet.\n\n"
        "Each input item includes the recent 'history' (last 10 messages, with roles "
        "user/bot/manager/system). MIRROR THE CUSTOMER'S TONE from that history — match their "
        "register, energy, emoji/punctuation level, and typical message length. Do not default to "
        "chipper or formal; continue naturally in the voice already established in the thread.\n\n"
