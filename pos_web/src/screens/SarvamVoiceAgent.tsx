...
export function SarvamVoiceAgent() {
  ...
  const addLog = useCallback((msg: string) => {
    ...
    console.log('[sarvam-voice-agent]', msg);
  }, []);
  ...
  useEffect(() => {
    ...
    const url = `${base}/ws/sarvam-voice/${outletId}?token=${token}`;
    setWsUrl(url);
    ...
    ws.onclose = (ev) => {
      ...
      else if (ev.code === 4003) setWsError('Backend: Sarvam API key not configured');
  ...
  return (
    <div style={{ padding: 16, maxWidth: 600, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <h2 style={{ margin: 0 }}>Sarvam Voice Agent</h2>
