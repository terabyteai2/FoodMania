          ...
          child: Column(
            children: [
              Icon(
                icon,
                size: 20,
                color: disabled ? PosColors.muted : PosColors.text,
              ),
              ...
              TfText(
                ...
                style: TextStyle(
                  fontSize: 11.5,
                  fontWeight: FontWeight.w600,
                  color: disabled ? PosColors.muted : PosColors.textSec,
                ),
              ),
            ],
          ),
...
class _MenuList extends StatelessWidget {
            ...
            child: Column(
              children: [
                for (int i = 0; i < grouped[category]!.length; i++) ...[
                  _buildRow(context, grouped[category]![i], i == 0),
                ],
              ],
            ),
          ),
  ...
  Widget _buildRow(BuildContext context, MenuItem item, bool first) {
    final text = AppScope.of(context).strings;
    final priceDecimals = item.price == item.price.roundToDouble() ? 0 : 2;
    final extras = item.extras;
    final discounted = extras.discountedPrice(item.price);
    ...
    final statusColor = item.isAvailable ? PosColors.good : PosColors.danger;
    ...
    return TfItemRow(
      ...
      desc: item.localizedDescription(language).trim().isEmpty
          ? item.localizedCategory(language)
          : item.localizedDescription(language),
      status: item.isAvailable ? text.menuAvailable : text.menuPaused,
      ...
      on: item.isAvailable,
      onChanged: (value) => onAvailabilityChanged(item, value),
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            TfText(text.menuAvailableForOrder),
                            const Spacer(),
                            TfToggle(
                              value: _isAvailable,
