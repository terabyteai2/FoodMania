...
/// Per-section diagnostics for a cloud pull (menu / orders / inventory).
/// Surfaces exactly what happened during a sync so a missing-data bug is
/// never silent: `pulled` = rows returned by the API, `applied` = rows written
/// to the local DB, `skipped` = rows dropped (parse/apply error), and
/// `lastError` = the most recent failure for this section.
class SyncSectionDiagnostics {
  const SyncSectionDiagnostics({
    this.pulled = 0,
    this.applied = 0,
    this.skipped = 0,
    this.lastError,
  ...
  SyncSectionDiagnostics copyWith({
    int? pulled,
    int? applied,
    int? skipped,
    String? lastError,
...
class SyncRuntimeState {
  SyncRuntimeState({
    ...
    this.lastSyncAt,
    this.lastError,
    this.menu = const SyncSectionDiagnostics(),
    this.orders = const SyncSectionDiagnostics(),
    this.inventory = const SyncSectionDiagnostics(),
  });
  ...
  final SyncSectionDiagnostics menu;
  final SyncSectionDiagnostics orders;
  final SyncSectionDiagnostics inventory;

  /// Derived: the first non-null section error from the last pull, or null if
  /// all sections succeeded. Useful for a single-line "last pull error" in the
  /// UI without having to inspect each section individually.
  String? get lastPullError =>
      menu.lastError ?? orders.lastError ?? inventory.lastError;
  ...
  SyncRuntimeState copyWith({
    ...
    SyncSectionDiagnostics? menu,
    SyncSectionDiagnostics? orders,
    SyncSectionDiagnostics? inventory,
    bool clearError = false,
  }) {
...
class SyncService {
  ...
  Future<void> syncNow() async {
    ...
    _state = _state.copyWith(
      isSyncing: true,
      clearError: true,
      menu: const SyncSectionDiagnostics(),
      orders: const SyncSectionDiagnostics(),
      inventory: const SyncSectionDiagnostics(),
    );
    _emitState();
    try {
      // The pre-pull handshake must NOT abort the whole sync. On desktop the
      // realtime (WebSocket) connect in particular can fail/throw without
      // affecting REST pulls — if it did, the entire menu/orders/inventory
      // pull would be skipped while settings/shift (pulled separately) still
      // loaded. Each step is isolated so one failure degrades gracefully.
      try {
        await _cloudApi.testHealth();
        _state = _state.copyWith(cloudConnected: true, clearError: true);
      } catch (error) {
        _addLog('Cloud health check failed: $error', isError: true);
        _state = _state.copyWith(
          cloudConnected: false,
          lastError: 'Cloud health check failed: $error',
        );
      }
      try {
        await _cloudApi.registerDevice();
      } catch (error) {
        _addLog('Device registration failed: $error', isError: true);
      }
      try {
        await _connectCloudRealtime();
      } catch (error) {
        _addLog('Realtime connect skipped (pulls continue): $error',
            isError: true);
      }
  ...
  Future<int> _pullCloudChanges() async {
    ...
    // Each network pull is isolated: a failure in one section must not blank
    // the others. This is the most likely cause of "menu + orders missing
    // while settings/shift load" — an aborted pull before the data was imported.
    List<Map<String, Object?>> menuPayloads = const [];
    List<Map<String, Object?>> orderPayloads = const [];
    Map<String, Object?> inventoryBundle = const {};
    String? menuError;
    String? ordersError;
    ...
    int menuApplied = 0;
    int menuSkipped = 0;
    int inventoryApplied = 0;
    int inventorySkipped = 0;

    try {
      menuPayloads = await _cloudApi.pullMenu(since: since);
    } catch (error) {
      menuError = error.toString();
      _addLog('Cloud menu pull failed: $error', isError: true);
    }
    ...
    try {
      orderPayloads = await _cloudApi.pullOrders(
        since: since,
        limit: since == null ? 500 : null,
      );
    } catch (error) {
      ordersError = error.toString();
      _addLog('Cloud orders pull failed: $error', isError: true);
    }
    try {
      inventoryBundle = await _cloudApi.pullInventory(since: since);
    } catch (error) {
      inventoryError = error.toString();
      _addLog('Cloud inventory pull failed: $error', isError: true);
    }
    var imported = 0;
    ...
    for (final payload in menuPayloads) {
      ...
      try {
        ...
        menuSkipped++;
        menuError = error.toString();
        final id = payload['id']?.toString() ?? 'unknown';
        final snippet = payload.toString().length > 300
            ? '${payload.toString().substring(0, 300)}…'
            : payload.toString();
        _addLog('Cloud menu import skipped ($id): $error', isError: true);
        if (kDebugMode) {
          debugPrint(
            '[QB-MENU-DIAG] skipped item id=$id error=$error payload=$snippet',
          );
        }
      }
    }
    if (kDebugMode) {
      debugPrint(
        '[QB-MENU-DIAG] pulled=$menuPulled applied=$menuApplied '
        'skipped=$menuSkipped error=$menuError',
      );
    }
    ...
    // Emit section diagnostics for the Settings panel
    _state = _state.copyWith(
      menu: SyncSectionDiagnostics(
        pulled: menuPulled,
        applied: menuApplied,
        skipped: menuSkipped,
        lastError: menuError,
      ),
      orders: SyncSectionDiagnostics(
        pulled: orderPayloads.length,
        applied: ordersApplied,
        skipped: ordersSkipped,
        lastError: ordersError,
      ),
      inventory: SyncSectionDiagnostics(
        pulled: (inventoryBundle['items'] is List
                ? (inventoryBundle['items'] as List).length
                : 0) +
            (inventoryBundle['suppliers'] is List
                ? (inventoryBundle['suppliers'] as List).length
                : 0) +
            (inventoryBundle['adjustments'] is List
                ? (inventoryBundle['adjustments'] as List).length
                : 0) +
            (inventoryBundle['dailyCounts'] is List
                ? (inventoryBundle['dailyCounts'] as List).length
                : 0),
    ...
    if (kDebugMode) {
      debugPrint(
        '[QB-PULL-DIAG] imported=$imported '
        'menu={pulled=$menuPulled applied=$menuApplied skipped=$menuSkipped error=$menuError} '
        'orders={pulled=${orderPayloads.length} applied=$ordersApplied skipped=$ordersSkipped error=$ordersError} '
        'inventory={pulled=${_state.inventory.pulled} applied=$inventoryApplied skipped=$inventorySkipped error=$inventoryError}',
      );
    }
