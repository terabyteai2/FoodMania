...
class LocalDatabaseService {
  ...
  Future<void> _migrateOrdersDropOrderNoUniqueV15(Database db) async {
    ...
    // Defensive: a previously failed run may have left orders_v15 behind.
    await db.execute('DROP TABLE IF EXISTS orders_v15');
    await db.execute('''
    ...
    ''');
    // Explicit column list: the legacy `orders` table is missing `orderDate`
    // (added later in the chain), so `SELECT *` would supply 32 values into 33
    // columns and crash the migration. Seed orderDate with NULL (it is a
    // nullable, not-yet-populated field) to keep column counts aligned.
    await db.execute('''
      INSERT INTO orders_v15 (
        id, orderNo, source, customerName, tableNo, note, deliveryAddress,
        mobileNumber, createdByAccountId, createdByRole, shiftId, discountLabel,
        discountAmount, serviceChargeRatePercent, serviceChargeAmount,
        billingSnapshot, kotBatches, settledAt, serviceType, covers,
        paymentMethod, subtotal, vatRatePercent, vatAmount, deliveryCharge,
        status, total, syncStatus, version, sequenceNo, orderDate, createdAt,
        updatedAt
      )
      SELECT
        id, orderNo, source, customerName, tableNo, note, deliveryAddress,
        ...
        billingSnapshot, kotBatches, settledAt, serviceType, covers,
        paymentMethod, subtotal, vatRatePercent, vatAmount, deliveryCharge,
        status, total, syncStatus, version, sequenceNo, NULL, createdAt,
        updatedAt
      FROM orders
    ''');
