...
// Ported verbatim from admin_app/windows/runner/flutter_window.cpp. The reused
// PrinterService (package:local_pos) talks to this native channel to send raw
// ESC/POS bytes to a Windows print queue via the spooler (datatype "RAW").
// The channel name MUST match the Dart side.
constexpr char kWindowsPrinterChannel[] =
...
bool PrintRawBytes(const std::string& printer_name,
  ...
  document.pDocName = const_cast<LPWSTR>(L"QuickBytes POS Receipt");
...
void RegisterWindowsPrinterChannel(flutter::FlutterEngine* engine) {
  ...
  channel = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
      engine->messenger(), kWindowsPrinterChannel,
...
bool FlutterWindow::OnCreate() {
  ...
  bt_printer::RegisterBtPrinterChannel(flutter_controller_->engine());
