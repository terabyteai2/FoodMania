...
- **Sidebar drawer (hamburger)** — the primary mobile/admin navigation pattern. Profile + outlet/account context at top, grouped nav with leading icons and `›` for groups, active row = soft `--primary-tint` + blue icon/label + optional left blue bar. Localized high-traffic labels. Rows are at least 44px tall.
- **Bottom tab bar** — not part of the current Flutter `admin_app` cleanup. Do not introduce bottom tabs unless a separate product decision explicitly asks for them.
...
## PART A2 — FLUTTER `admin_app` IMPLEMENTATION CONTRACT

> This section makes `DESIGN.md` directly implementable for the Flutter `admin_app`. It is the source of truth for the **mobile/tablet admin surface** cleanup. Preserve existing behavior, routing, roles, data loading, and workflows. The desktop/POS workstation guidance in Part C remains reference material and is **out of scope** for this mobile admin design-system pass.

### 7a. Token mapping

Use the existing Flutter token layer (`AppTheme`, `PosColors`, `PosSpacing`, `PosRadii`, `PosShadows`, and shared `Tf*` widgets) as the implementation surface. Do not create a competing design-system namespace.

**Colors → `PosColors`**
- Primary: `primary #2F4FE0`, `primaryDeep #2440C2`, `primarySoft #EEF1FE`, `primaryWash #DCE3FD`, `accentStrong #1D339B`, `accentInk #FFFFFF`.
- Secondary/navy action: `secondary #1E2A44`, `secondaryPress #15203A`, `onSecondary #FFFFFF`.
- Neutrals: `background #F6F7F9`, `surface #FFFFFF`, `surfaceSunk #F1F3F6`, `surface3 #E7EAEF`, `primaryDark #1B2330`, `inkSoft #5A6475`, `muted #8A93A3`, `mutedSoft #AAB2C0`, `line #ECEEF2`, `lineStrong #DEE2E9`.
- Signals: `success #1E9E5A` / `successSoft #E3F6EC`, `warning #C98208` / `warningSoft #FCF0D8`, `danger #D8434A` / `dangerSoft #FBE5E6`, `info = primary`.
- State fills: running/seated `stateRunning #DCEBFB` + `stateRunningInk #2B5C9E`; printed `statePrinted #E3F6EC` + `statePrintedInk #1E7A47`; KOT `stateKot #FCF3CF` + `stateKotInk #9A7400`; available = `surface` with `lineStrong`.
- Green is success/profit only. Red is destructive/error only. Navy is secondary action only. Blue is the only primary/action/active color.

**Spacing → `PosSpacing`**
- Base-4 scale: `sp1=4`, `sp2=8`, `sp3=12`, `sp4=16`, `sp5=20`, `sp6=24`, `sp7=32`, `sp8=40`.
- Mobile screen padding defaults to `sp4`; compact panel/list gaps use `sp2-sp3`; card padding uses `sp3-sp4`; section gaps use `sp3-sp5`.

**Radii → `PosRadii`**
- `sm=8`, `md=12`, `lg=14`, `xl=16`, `pill=999`.
- Aliases: `card=lg`, `input=md`, `tile=lg`, `chip=pill`, `toggle=pill`, `tag=sm`.
- Cards and tiles use 14-16px. Inputs and buttons use 12px. Chips, segmented active pills, toggles, status pills, and FABs use pill/circle geometry.

**Elevation → `PosShadows`**
- `card`: `0 1px 2px rgba(20,25,40,.04), 0 2px 8px rgba(20,25,40,.05)` plus a 1px `line` border.
- `e3`: dialogs, bottom sheets, dropdowns, and floating detail popovers.
- `bar` / `eUp`: sticky footer action bars, `0 -2px 12px rgba(20,25,40,.06)`.
- `fab`: `0 8px 22px -6px rgba(47,79,224,.55)`.

### 7b. Mobile admin navigation contract

- The Flutter `admin_app` keeps the existing **sidebar drawer** as primary mobile/tablet admin navigation. Do not replace it with bottom tabs during design-system cleanup.
- Top bar keeps hamburger/back, screen title, outlet subtitle where relevant, notification bell, Messages/account actions, and page-specific trailing actions.
- Drawer surface is `surface` with a 1px `line` edge/border. Active destination uses `primarySoft`, `primary` icon/label, and an optional 3px blue left bar. Inactive rows use neutral ink/muted text.
- Drawer rows are at least 44px tall with leading icons, localized high-traffic labels, and grouped sections. Parent/group rows may use `›`; destructive sign-out uses red text/icon only.
- Current role-aware nav order remains behavior-owned by the app. The design system styles the rows and states; it does not change route availability.

### 7c. Exhaustive component specs for Flutter cleanup

**Buttons**
- Primary button: h48 (h52 for large/footer), radius `md`, `primary` fill, `accentInk` text/icon, 600-700 weight. Only one visually dominant primary action per screen zone.
- Secondary button: same metrics, `secondary` fill, `onSecondary` text/icon. Use as the left half of paired footers, never as the main primary action by itself.
- Ghost/outline: transparent or `surface`, 1px `lineStrong`, neutral ink text. Blue-outline variant uses `primary`/`accentStrong` text for non-destructive links like View Invoice.
- Destructive: red text/icon or red outline only; solid red is reserved for serious destructive confirmations. Minus/remove steppers are neutral unless the action is true deletion.
- Disabled: `line` background/border, `muted` text/icon; keep minimum target 44px.
- Paired footer: sticky `eUp` surface, buttons side-by-side on mobile (`flex:1`), navy secondary on left and blue primary on right.

**Cards and panels**
- Default card: `surface`, radius `lg`, 1px `line`, `PosShadows.card`, padding `sp3-sp4`.
- Cards are for repeated items, panels, dialogs, and framed tools. Avoid decorative card-inside-card layouts.
- Stat cards use icon tint circles, tabular bold numbers, small muted labels, and optional chevron. Mobile grids are 2-3 columns.
- Report sections use white cards, bold 15-16px headers, hairline row dividers, label-left/value-right rows, parenthesized deductions, and bold totals.

**Inputs and forms**
- Text fields/search: h48, radius `md`, `surface` or `surfaceSunk`, 1px `lineStrong`, leading icon where useful, focus border/ring = `primary` or `accentStrong`.
- Labels use `muted`/`inkSoft`; placeholder uses `mutedSoft`; helper text is 12-13px muted; error text and borders use `danger`.
- Form rows use token spacing and `min-width:0`/flex-safe layout so Bangla and long values do not overflow.
- Radios, checkboxes, switches, toggles: selected/on state uses `primary`; off state uses neutral track/border.

**Tabs, chips, and filters**
- Primary navigation tabs are underline tabs: active blue label + 2-3px blue underline; inactive muted.
- Analytics two-tab toggle is a segmented control on `surfaceSunk`, active segment = `surface` + blue text + subtle card shadow.
- Chips are pill-shaped. Active filled chip = `primary` + white; soft active chip = `primarySoft` + `accentStrong`; inactive chip = `surface` + `lineStrong`.

**Tables and lists**
- Dense list rows are at least 56px high, `surface` with hairline `line` dividers, 14-15px row titles, 13px muted metadata, tabular money on the trailing edge.
- Table-like data may use zebra rows alternating `surface`/`surfaceSunk`. Header rows use 12-13px 600 labels and muted/sort icons.
- Sortable headers, sticky headers, total rows, and row overflow menus should be used where existing behavior already supports them; do not add new data behavior during visual cleanup.
- State/status labels use pastel fill + darker ink; never saturated blocks.

**Dialogs, sheets, and snackbars**
- Dialogs: `surface`, radius `xl`, 1px `line`, `e3` shadow, title 18px 600-700, content 14px, actions aligned with primary/secondary hierarchy.
- Bottom sheets: top radius `xl`, drag handle, `e3` shadow, compact `sp4` horizontal padding.
- Snackbars/toasts: floating, ink background, white text, radius `md`. Success/error variants use leading icon/accent or small state detail; do not turn the whole snackbar green/red unless it is a narrow critical alert.

**Empty, loading, and error states**
- Use shared primitives (`TfEmptyState`, `TfLoading`, `ErrorView` or direct equivalents) before custom screen-specific states.
- Icon container uses a tokenized soft fill (`primarySoft` for empty/loading, `dangerSoft` for errors), radius `lg`, token border, and muted supporting copy.
- Empty states have title 16-18px 700, body 13-14px muted, optional primary/ghost action. Loading uses primary progress. Errors use danger icon/text and a retry action when available.

**Typography**
- Font stack: Inter primary, Hind Siliguri/Noto Sans Bengali fallback. Use `TfText`/safe text helpers for localized high-traffic labels.
- Use the roles in §3. Money, counts, order serials, and analytics numbers use tabular figures.
- Letter spacing is 0 by default. Eyebrows may use 0.05em uppercase in English. Avoid negative letter spacing except numeric hero styles that are already verified safe for Bangla.

### 7d. Hard-coded value policy

- Replace reusable hard-coded colors, font sizes, paddings, margins, border radii, borders, and shadows with centralized tokens or shared component styles.
- Local numeric literals are allowed for layout mechanics only: icon sizes, fixed avatar/image sizes, chart geometry, breakpoint thresholds, animation durations, one-off constraints, and package/API-required values.
- No raw `Color(0x...)` outside token/theme/category-tint/chart-palette files unless the value is explicitly documented as data visualization or category color.
- Remove or update stale comments that refer to old “lime”, “plum”, “crisp square”, or previous source-of-truth docs in admin UI files.
- Preserve app behavior and data flow. A visual cleanup must not change routing, API contracts, persistence, roles/permissions, order state transitions, or analytics calculations.

### 7e. Flutter cleanup checklist

- `AppTheme.light()` uses this file's token values and Material defaults for buttons, inputs, cards, chips, dialogs, snackbars, switches, checkboxes, radios, progress indicators, sliders, and tooltips.
- Shared primitives consume tokens: `TfButton`, `TfCard`, `TfSearchField`, `TfTabs`, `TfSegToggle`, `TfEmptyState`, `TfLoading`, `ErrorView`, app top bar, and sidebar drawer.
- Mobile admin screens preserve existing workflows and data flow.
- Sidebar drawer remains the mobile admin navigation pattern.
- Primary action is blue; paired secondary action is navy; red appears only for destructive/error; green only for success/profit.
- Cards use radius 14-16; inputs/buttons use radius 12; chips/toggles use pill geometry.
- Search and forms focus with primary blue.
- Empty/loading/error states use shared components or match their tokenized specs.
- Tables/lists use tokenized row padding, dividers, text styles, and state fills.
- Formatting passes with `dart format`.
- Available checks pass with `flutter analyze` and existing `flutter test`, or failures are documented with exact causes.
- Remaining manual review areas are listed by screen when any raw values remain for visual judgment.
...
>
> **Flutter `admin_app` cleanup scope:** desktop/POS workstation guidance is reference-only for the current mobile/tablet admin design-system audit. Do not implement or refactor desktop rail, desktop top bar, or POS workstation surfaces as part of that cleanup unless a separate task explicitly requests it.
...
| Nav | Sidebar drawer + top bar | Top app bar + apps-grid launcher | Icon rail + top bar |
