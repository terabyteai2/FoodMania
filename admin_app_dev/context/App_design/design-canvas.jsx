  ...
  React.useEffect(() => {
    ...
    const onHostMsg = (e) => {
      ...
      if (d && d.type === '__dc_set_zoom' && typeof d.scale === 'number') {
        // Set scale directly — no zoomAt so the saved pan position is preserved
        // and the drift-correction path (which can send y far off-screen on
        // layouts with large --dc-inv-zoom reflows) never runs on load.
        tf.current.scale = Math.min(maxScale, Math.max(minScale, d.scale));
        apply();
