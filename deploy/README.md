...
Keep backend secrets in local `backend/.env` and the VPS env file, never in Git. After rotating Scan Menu keys, set `OCR_SPACE_API_KEY`, `DEEPSEEK_API_KEY`, and any OCR or `MENU_SCAN_*_MODEL` overrides locally, then merge the VPS-safe values and restart the service with:
