...
class PosAppController extends ChangeNotifier {
  ...
  /// Set when the terminal printer is unavailable — blocks auto-print for all orders.
  ...
  /// One in-app alert per infrastructure error message.
  ...
  Future<void> reloadData() async {
    ...
    inventorySuppliers = const [];
    inventoryTodaySpend = 0;
  ...
  Future<void> refreshInventorySummary() async {
    ...
    inventorySummaryError = 'Inventory is disabled in Terminal app.';
    notifyListeners();
  }

  Future<DailyReport> fetchDailyReport({DateTime? date}) {
    return Future.error(
      CloudApiException('Inventory is disabled in Terminal app.'),
    );
  }
  ...
  void _handleRemoteSyncEvent(Map<String, Object?> event) {
    ...
    if (type == 'terminal_app_update_disabled') {
    ...
    if (type != 'terminal_app_update_available' || data is! Map) return;
    unawaited(
      _considerAppUpdate(
