...
@pytest.mark.asyncio(loop_scope="session")
async def test_order_details_patch_payment_method_is_null_safe():
    await create_tables()
    server_id = f"order-payment-patch-{uuid.uuid4()}"
    order_id = f"order-{uuid.uuid4()}"
    transport = ASGITransport(app=app)
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        bootstrap = await client.post(
            ...
            json={
                "serverId": server_id,
                "restaurantName": "Payment Patch Test",
                "tableCount": 4,
            },
        )
        data = bootstrap.json()["data"]
        ...
        # New client: PATCH carries paymentMethod and persists it.
        patched = await client.patch(
            f"/outlets/{outlet_id}/orders/{order_id}",
            headers=headers,
            json={"paymentMethod": "nagad"},
        )
        # Legacy client: PATCH without paymentMethod must NOT wipe the value.
        legacy = await client.patch(
            f"/outlets/{outlet_id}/orders/{order_id}",
            headers=headers,
            json={"customerName": "Ayesha Khan"},
        )
        # Split settlements are stored as the sentinel "split" (pos settle).
        split_patch = await client.patch(
            f"/outlets/{outlet_id}/orders/{order_id}",
            headers=headers,
            json={"paymentMethod": "split"},
        )
        pulled = await client.get(f"/outlets/{outlet_id}/orders", headers=headers)

    assert created.status_code == 200
    assert created.json()["data"]["paymentMethod"] is None

    assert patched.status_code == 200
    assert patched.json()["data"]["paymentMethod"] == "nagad"

    assert legacy.status_code == 200
    assert legacy.json()["data"]["paymentMethod"] == "nagad"
    assert legacy.json()["data"]["customerName"] == "Ayesha Khan"

    assert split_patch.status_code == 200
    assert split_patch.json()["data"]["paymentMethod"] == "split"

    assert pulled.status_code == 200
    pulled_order = next(o for o in pulled.json()["data"] if o["id"] == order_id)
    assert pulled_order["paymentMethod"] == "split"
