...
class VoiceAgentSession:
    ...
    async def _stream_tts_sarvam(self, text: str):
        ...
        logger.info("[voice:tts] Streaming via Sarvam AI (model=%s speaker=%s pace=%s temperature=%s)",
        ...
        try:
            ...
            logger.warning("[voice:tts] SDK too old for prosody params, falling back to defaults")
            chunks = client.text_to_speech.convert_stream(
                model=settings.SARVAM_TTS_MODEL,
                text=text,
                target_language_code=settings.SARVAM_TTS_LANGUAGE,
